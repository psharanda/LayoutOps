    ...
    fileprivate func calculateValue<T: BoxDimension>(forViews views: [Layoutable]?, dimension: T) -> CGFloat {
        ...
        case .first:
            if let views = views, views.count > 0 {
                return T.getDimension(views[0].lx_frame).size
            } else {
                return 0
            }
        case .max:
            if let views = views, views.count > 0 {
                return views.reduce(CGFloat.leastNormalMagnitude) { Swift.max($0, T.getDimension($1.lx_frame).size) }
            } else {
                return 0
            }
        case .min:
            if let views = views, views.count > 0 {
                return views.reduce(CGFloat.greatestFiniteMagnitude) { Swift.min($0, T.getDimension($1.lx_frame).size) }
            } else {
                return 0
            }
        case .value(let v):
